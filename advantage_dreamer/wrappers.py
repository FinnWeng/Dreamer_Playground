...
class Atari:
  ...
  def __init__(self, name, action_repeat=4, size=(84, 84), grayscale=True, noops=30,life_done=False, sticky_actions=True):
    ...
    self._noops = noops # randomly play for first several steps
    self._life_done = life_done # done for live down for 1
    self._lives = None
    shape = self._env.observation_space.shape[:2] + (() if grayscale else (3,))
    self._buffers = [np.empty(shape, dtype=np.uint8) for _ in range(2)] # to save observation to gray scale or others. Not replay buffer.
  ...
  def step(self, action):
    total_reward = 0.0
    for step in range(self._action_repeat): # every action repeat for 4 times
      _, reward, done, info = self._env.step(action)
      total_reward += reward # the total reward for 1 step, 4 same actions
      if self._life_done:
      ...
      if done:
        break
      elif step >= self._action_repeat - 2: # take the last 2 frame
...
class Collect:
  # wrap env to collect episode data for every step
  ...
  def step(self, action):
    ...
    obs = {k: self._convert(v) for k, v in obs.items()} # convert dtype of obs
    transition = obs.copy()
    transition['action'] = action
    transition['reward'] = reward # the total reward for 1 step, 4 same actions
    transition['discount'] = info.get('discount', np.array(1 - float(done)))
    self._episode.append(transition) # save every step(transition) to episode list
    if done:
      episode = {k: [t[k] for t in self._episode] for k in self._episode[0]} # {k: list of value}
      episode = {k: self._convert(v) for k, v in episode.items()}
      info['episode'] = episode
      for callback in self._callbacks:
        # one for save, one for tf.summary and show
        callback(episode)
    return obs, reward, done, info # so the reward is still for only one step
...
class TimeLimit:
  ...
  def step(self, action):
    ...
    obs, reward, done, info = self._env.step(action)
    self._step += 1
    if self._step >= self._duration: # close this episode if it step across the time limit
      done = True
...
class OneHotAction:
  ...
  def step(self, action):
    ...
    if not np.allclose(reference, action):
      raise ValueError(f'Invalid one-hot action:\n{action}') # check the action is the same as reference
...
class RewardObs:
  # append reward to obs. Can't get the point to do it seperately.
...
class Async:
  ...
  def call(self, name, *args, **kwargs):
    ...
    if self._strategy == 'none':
      # the getattr(self.env, step) will reture a step function of gym env. send anction in *args to step to do action
  ...
  def _worker(self, ctor, conn): # the function that should be execute by processes,
    try:
      env = ctor()
      while True:
        try:
