...
class Module(tf.Module):
  # wrap tensorlfow module with handy functions
...
def simulate(agent, envs, steps=0, episodes=0, state=None):
  '''
  this function let 
    dreamer to dream(world model) and learn(actor critic)
    random agent interact with gym
  '''
  ...
  while (steps and step < steps) or (episodes and episode < episodes):
    ...
    action, agent_state = agent(obs, done, agent_state) # every time the agent(dreamer) is been called, it will imagine one step forward.
...
def lambda_return(reward, value, pcont, bootstrap, lambda_, axis):
  ...
  if axis != 0:
    ...
    pcont = tf.transpose(pcont, dims) #  self._c.discount * tf.ones_like(reward) for c.pcont = False
  if bootstrap is None:
    bootstrap = tf.zeros_like(value[-1])
  next_values = tf.concat([value[1:], bootstrap[None]], 0)
  inputs = reward + pcont * next_values * (1 - lambda_) # the reward is from world model, and the value is predicted by critic
  print("inputs:",inputs.shape) # inputs: (14, 1225)
  print("pcont:",pcont.shape) # pcont: (14, 1225)
  print("bootstrap:",bootstrap.shape) # bootstrap: (1225,)
  returns = static_scan(
      lambda agg, cur: cur[0] + cur[1] * lambda_ * agg,
      (inputs, pcont), bootstrap, reverse=True) # bootstrap:value[-1] which is no.15 , lambda:0.95
  if axis != 0:
...
def save_episodes(directory, episodes): # episode: {k: list of value}
  directory = pathlib.Path(directory).expanduser()
  directory.mkdir(parents=True, exist_ok=True)
  timestamp = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
  for episode in episodes:
    identifier = str(uuid.uuid4().hex)
    length = len(episode['reward']) # the total reward for 1 step, 4 same actions
      ...
      if filename not in cache:
        try:
          with filename.open('rb') as f:
            ...
            episode = {k: episode[k] for k in episode.keys()} # dict_keys(['image', 'action', 'reward', 'discount'])
    ...
    for index in random.choice(len(keys), rescan):
      ...
      if length:
        ...
        if balance:
          ...
          index = int(random.randint(0, available)) # randomly choose 0~available samples of  batch_length traj
...
def static_scan(fn, inputs, start, reverse=False):
  ...
  observe:
    fn: lambda prev, inputs: self.obs_step(prev[0], *inputs), so the inputs is (state, action, embed)
    inputs: (action, embed)
    start: (state, state)
  ...
  embed  (50, 25, 1024) batch_length:50 ,batch_size 50/2 = 25
  action (50, 25, 4)

  so the fn is executing step by step, and input is a batch, and out is a batch of prior/(post,prior)
  '''
  last = start

  outputs = [[] for _ in tf.nest.flatten(start)]
  indices = range(len(tf.nest.flatten(inputs)[0])) # through axis 1
  if reverse:
    indices = reversed(indices)
  print("indices:",indices)
  for index in indices: # loop through batch_length or horizon
...
def tuple_static_scan(fn, inputs, start, reverse=False):
  '''
  fn(start,inputs[n])
  ...
  outputs = [[] for _ in tf.nest.flatten(start)] # to shape of dict
  action_outputs = []
  ...
  for index in indices: # loop through batch_length or horizon
    ...
    [o.append(l) for o, l in zip(outputs, tf.nest.flatten(last[0]))]
    action_outputs.append(last[1])
    
  # print("action_outputs:",len(action_outputs))    
  if reverse:
    outputs = [list(reversed(x)) for x in outputs]
  outputs = [tf.stack(x, 0) for x in outputs]
  action_outputs = tf.stack(action_outputs, 0)
  # print("outputs:",len(outputs))
  # print("outputs:",outputs) # list of tensor, each is (15, 1225, number)
  return tf.nest.pack_sequence_as(start, outputs), action_outputs #pack_sequence_as: pack back to dict
